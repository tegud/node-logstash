...
function formatResults( row ) {
	...
	for (var column in row) {
		switch(column.toLowerCase()) {
			...
			case 'url':
				var matches = row.url.match(/[^server=]+server=([^&|^$]+).*/ig);
			...
			case 'text':
				var exception = stacktrace_parser.parse(row[column], function(exception) {
					data.message = (exception.type ? exception.type + ': ' : '') + exception.error;

					for (var detail in exception) {
