...
function InputWmi() {
  ...
  this.config = {
    name: 'WMI',
    host_field: 'server',
	optional_params: ['type', 'root', 'outputClassName'],
...
InputWmi.prototype.afterLoadConfig = function(callback) {
	...
	this.monitor = monitor_wmi.create(this.server, this.root, 500, this.outputClassName);
	...
	this.monitor.query( q, function( data ) {
		data.metricGroup = this.outputClassName;
		this.emit('data', {
